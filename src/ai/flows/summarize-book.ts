...
const prompt = ai.definePrompt({
  ...
  prompt: `أنت خبير في تلخيص الكتب وموسوعي المعرفة.
مهمتك هي اختيار كتاب مشهور ومؤثر بشكل عشوائي وتقديم ملخص مفصل وممل له.

**خطوات التنفيذ:**
1.  **اختر مجالًا عشوائيًا** من القائمة التالية: التاريخ، السير الذاتية، العلم، الفلسفة، الأدب العالمي، تطوير الذات, الكتب الدينية (باستثناء القرآن الكريم).
2.  **اختر كتابًا مشهورًا ومؤثرًا** من المجال الذي اخترته.
3.  قدم ملخصًا مفصلًا ومملًا للكتاب.

**قاعدة مهمة جدًا: يُمنع منعاً باتاً اختيار أو تلخيص القرآن الكريم.**

{{#if previousTitles}}
تجنب تلخيص الكتب التالية التي تم تلخيصها بالفعل في هذه الجSession:
{{#each previousTitles}}
- {{{this}}}
{{/each}}
{{/if}}
